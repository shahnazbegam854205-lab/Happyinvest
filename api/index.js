...
admin.initializeApp({
    ...
    databaseURL: process.env.FIREBASE_DATABASE_URL || "https://happy-invest-default-rtdb.firebaseio.com"
});
...
const checkAdmin = (req, res, next) => {
    ...
    const correctPassword = process.env.ADMIN_PASSWORD || 'happyinvest123';
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const userData = {
            ...
            level: 1,
            teamCount: 0,
            referralLink: `https://happy-invest.com/register?ref=${userReferralCode}`,
            bankDetails: null,
            dailyCheckin: {
                lastCheckin: null,
                streak: 0
            ...
            try {
                ...
                if (referrerSnapshot.exists()) {
                    referrerSnapshot.forEach(async (childSnapshot) => {
                        ...
                        await db.ref(`commissions/${referrerId}/${commissionId}`).set({
                            id: commissionId,
                            fromUserId: userId,
                            fromUserName: name,
                            amount: 0, // Initial commission will be added when user recharges
                            type: 'referral',
                            level: 1,
...
app.post('/api/invest', async (req, res) => {
    try {
        ...
        // Get plan details
        const plans = {
            "wind_1": { price: 800, dailyIncome: 279, totalIncome: 6138, duration: 22 },
            "wind_2": { price: 560, dailyIncome: 1447, totalIncome: 13023, duration: 9 },
            "wind_3": { price: 1000, dailyIncome: 3900, totalIncome: 39000, duration: 10 },
            "wind_4": { price: 1600, dailyIncome: 4730, totalIncome: 28380, duration: 6 },
            "wind_5": { price: 1600, dailyIncome: 7350, totalIncome: 58800, duration: 8 },
            "wind_6": { price: 2800, dailyIncome: 17687, totalIncome: 70748, duration: 4 },
            "wind_7": { price: 5000, dailyIncome: 32600, totalIncome: 32600, duration: 1 },
            "wind_power_a": { price: 450, dailyIncome: 1830, totalIncome: 5490, duration: 3 },
            "wind_power_b": { price: 900, dailyIncome: 3900, totalIncome: 15600, duration: 4 }
        };
    ...
    try {
        ...
        res.json({ 
            ...
            stats: {
                ...
                level1Recharges: level1Recharges,
                level2Recharges: 0,
                level3Recharges: 0
            }
        });
...
app.listen(PORT, () => {
    ...
    console.log(`   POST   /api/recharge          - Create recharge request`);
    console.log(`   POST   /api/withdraw          - Create withdrawal request`);
    console.log(`   GET    /api/plans             - Get investment plans`);
    console.log(`   POST   /api/checkin           - Daily check-in`);
    console.log(`   POST   /api/admin/*           - Admin endpoints`);
